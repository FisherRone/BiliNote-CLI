...
class UniversalGPT(GPT):
    def __init__(self, client, model: str, temperature: float = 0.7):
        ...
        # 初始化时缓存重试配置
        self._max_retry_attempts = max(1, config_mgr.get("gpt_client.retry_attempts", 3))
        self._retry_base_backoff = float(config_mgr.get("gpt_client.retry_backoff_seconds", 1.5))
    ...
    def _checkpoint_path(self, checkpoint_key: str) -> Path:
        """获取检查点文件路径"""
        return Path(self.path_manager.get_gpt_checkpoint_path(checkpoint_key))
