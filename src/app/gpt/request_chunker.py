...
class RequestChunker:
    def __init__(self, message_builder: Callable, max_size: int, size_estimator: Optional[Callable] = None):
        ...
        if not segments:
            for image in image_urls:
                ...
                for chunk in chunks[-1:]:
                    ...
                    if self._messages_size(chunk.segments, candidate_images, **kwargs) <= self.max_size:
                ...
                if self._messages_size([], [image], **kwargs) > self.max_size:
                    raise ValueError("single image payload exceeds max_size")
        ...
        while idx < len(texts):
            ...
            while idx < len(texts):
                ...
                if size <= self.max_size:
                    ...
                    continue
                if not group:
                    raise ValueError("single text block exceeds max_size")
                break
            groups.append(group)
        return groups
