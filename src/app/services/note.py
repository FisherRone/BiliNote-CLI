...
from app.transcriber.transcriber_provider import get_transcriber_with_fallback, _transcribers
...
class NoteGenerator:
    ...
    def __init__(self):
        ...
        self.model_size: str = config_manager.get("transcriber.whisper_model_size", "base")
        self.device: Optional[str] = None
        self.transcriber_type: str = config_manager.get("transcriber.default_type", "fast-whisper")
    ...
    def generate(
        ...
        :param model_name: GPT 模型名称（如 gpt-4o, deepseek-chat）
        ...
        :param output_path: 笔记输出文件路径（可选，覆盖默认路径）
        ...
        try:
            ...
            # 缓存文件路径
            audio_cache_file = Path(path_manager.get_audio_meta_cache_path(task_id))
            transcript_cache_file = Path(path_manager.get_transcript_cache_path(task_id))
            # 如果指定了 output_path，使用它作为最终输出路径；否则使用默认路径
            if output_path:
                markdown_cache_file = Path(output_path)
                # 确保输出目录存在
                markdown_cache_file.parent.mkdir(parents=True, exist_ok=True)
            else:
                markdown_cache_file = Path(path_manager.get_note_output_path(task_id))
    ...
    def _init_transcriber(self) -> Transcriber:
        """
        根据配置动态获取并实例化转写器（带自动 fallback 功能）
        """
        logger.info(f"初始化转写器，首选类型：{self.transcriber_type}")
        return get_transcriber_with_fallback()

    def _get_gpt(self, model_name: Optional[str]) -> GPT:
        """
        根据 model_name 获取对应的 GPT 实例
        :param model_name: 模型名称（同时作为配置标识符）
        :return: GPT 实例
        """
        ...
        config = ModelConfig(
            ...
            provider="openai",  # 所有兼容 OpenAI 接口的都使用这个
            name=model_name,
        )
    ...
    def _save_metadata(self, video_id: str, platform: str, task_id: str) -> None:
        ...
        try:
            ...
            logger.info(f"已保存任务元数据 (video_id={video_id}, platform={platform}, task_id={task_id})")
        except Exception as e:
